...
public partial class CopyScene
{
    ...
    static void TerrainDataAssetFileReWrite(TerrainData terrain, FileInfo fileinfo)
    {
        ...
        if (layerArray != null)
        {
            for (int i = 0; i < layerArray.Length; i++)
            {
                ...
                var absSrcPath = EditorUtils.AssetsPath2ABSPath(assetSrcPath);
                ...
                var assetDstFile = EditorUtils.ABSPath2AssetsPath(absDstFile);
    ...
    static bool AssetFileReWrite(FileInfo fileinfo)
    {
        ...
        var assetPath = EditorUtils.ABSPath2AssetsPath(absPath);
    ...
    static void DelFileListProc()
    {
        ...
        {
            var assetPath = EditorUtils.ABSPath2AssetsPath(file);
    ...
    static void DstFileAnalyse(string dstfile)
    {
        ...
        foreach(var depend in depends)
        {
            ...
            var assetSrcDir = EditorUtils.ABSPath2AssetsPath(m_SrcDir);
            var isSrcDir = depend.StartsWith(assetSrcDir);
            if (!isSrcDir)
                continue;

            var abspath = EditorUtils.AssetsPath2ABSPath(depend);
    ...
    static void CopyFolder2()
    {
        ...
        foreach (var srcfile in m_SrcFileList)
        {
            ...
            var assetSrcFile = EditorUtils.ABSPath2AssetsPath(srcfile);
            var assetDstFile = EditorUtils.ABSPath2AssetsPath(dstfile);
